...
class CredentialManager:
    ...
    def load_credentials_list(self):
        ...
        log(
            "info",
            f"找到 {len(self.credentials_files)} 个凭证文件: {[os.path.basename(f) for f in self.credentials_files]}",
        )
        return True
...
def init_vertex_ai():
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    if missing_fields:
                        ...
                        raise ValueError(
                            f"Credentials JSON 缺少必需字段: {missing_fields}"
                        )
                except json.JSONDecodeError as json_err:
                    log(
                        "error",
                        f"vertex: 无法将 GOOGLE_CREDENTIALS_JSON 解析为 JSON: {json_err}",
                    )
                    raise
                ...
                try:
                    ...
                    log(
                        "info",
                        f"使用 GOOGLE_CREDENTIALS_JSON 环境变量初始化 Vertex AI 用于项目: {project_id}",
                    )
                ...
                log(
                    "error",
                    f"vertex：从 GOOGLE_CREDENTIALS_JSON 加载凭证时发生错误: {e}",
                )
                # 如果这里失败，继续尝试其他方法
        ...
        log(
            "info",
            f"尝试使用凭证管理器从文件获取凭证 (目录: {credential_manager.credentials_dir})",
        )
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    try:
                        ...
                        log(
                            "info",
                            f"Initialized Vertex AI using GOOGLE_APPLICATION_CREDENTIALS file path for project: {project_id}",
                        )
                        return True
                    except Exception as client_err:
                    ...
                    log(
                        "error",
                        f"vertex: 从 GOOGLE_APPLICATION_CREDENTIALS 路径 {file_path} 加载凭证时发生错误: {e}",
                    )
            else:
                log(
                    "error",
                    f"vertex: GOOGLE_APPLICATION_CREDENTIALS 文件不存在于路径: {file_path}",
                )

        # 如果没有任何方法成功
        log(
            "error",
            f"vertex: 没有找到有效的凭证。尝试了 GOOGLE_CREDENTIALS_JSON, 凭证管理器 ({credential_manager.credentials_dir}), 和 GOOGLE_APPLICATION_CREDENTIALS.",
        )
        return False
    except Exception as e:
        log(
            "error",
            f"vertex: 初始化认证时发生错误，如果您不使用Vertex，请忽略这些错误: {e}",
        )
        return False
...
def create_gemini_prompt(
    ...
    if not gemini_messages:
        ...
        return types.Content(
            role="user", parts=[types.Part(text="Error: No valid messages")]
        )
...
def convert_to_openai_format(gemini_response, model: str) -> Dict[str, Any]:
    ...
    for i, choice in enumerate(choices):
        if hasattr(gemini_response, "candidates") and i < len(
    ...
    try:
        ...
        if is_auto_model:
            ...
            if base_model_name != "gemini-2.5-flash-preview-04-17":
                error_response = create_openai_error_response(
                    400,
                    f"Model '{request.model}' does not support -nothinking variant",
            ...
            if base_model_name != "gemini-2.5-flash-preview-04-17":
                error_response = create_openai_error_response(
                    400,
                    f"Model '{request.model}' does not support -max variant",
                    "invalid_request_error",
                )
                return JSONResponse(status_code=400, content=error_response)
        else:
        ...
        def is_response_valid(response):
            ...
            log(
                "error",
                f"无效的响应: 在响应结构中没有找到文本内容: {str(response)[:200]}...",
            )
            return False
        ...
        async def make_gemini_call(model_name, prompt_func, current_gen_config):
            ...
            if request.stream:
                    ...
                    async def stream_generator_inner():
                        async def generator():
                            try:
                                log(
                                    "info",
                                    f"向 Gemini API 发送请求 (Model: {model_name}, Prompt Format: {prompt_func.__name__}), Fake streaming ({settings.FAKE_STREAMING_INTERVAL})",
                                )
                        ...
                        generate = asyncio.create_task(
                            generator(), name="generate_content"
                        )
                        timeout = asyncio.create_task(
                            asyncio.sleep(300), name="timeout"
                        )
                        try:
                            is_done = False
                            while not is_done:
                    ...
                    async def stream_generator_inner():
                        ...
                        try:
                            for candidate_index in range(candidate_count):
                                log(
                                    "info",
                                    f"向 Gemini API 发送流式请求 (Model: {model_name}, Prompt Format: {prompt_func.__name__})",
                                )
                                responses = (
                            ...
                            if not first_chunk_received:
                                raise ValueError(
                                    "Stream connection established but no chunks received"
                                )

                            yield create_final_chunk(
                                request.model, response_id, candidate_count
                            )
                            yield "data: [DONE]\n\n"

                            # Return status based on content received
                            if (
                                all_chunks_empty and first_chunk_received
                            ):  # Check if we got chunks but they were all empty
                                raise ValueError(
                                    "Streamed response contained only empty chunks"
                                )  # Treat empty stream as failure for retry
                ...
                return StreamingResponse(
                    stream_generator_inner(), media_type="text/event-stream"
                )
                ...
                try:
                    log(
                        "info",
                        f"向 Gemini API 发送请求 (Model: {model_name}, Prompt Format: {prompt_func.__name__})",
                    )
                    response = await client.aio.models.generate_content(
                        model=model_name,
                        contents=prompt,
        ...
        if is_auto_model:
            ...
            attempts = [
                ...
                },
                {
                    "name": "old_format",
                    "model": base_model_name,
                    "prompt_func": create_gemini_prompt_old,
                    "config_modifier": lambda c: c,
                },
                {
                    "name": "encrypt",
                    ...
                    "config_modifier": lambda c: {
                        **c,
                        "system_instruction": encryption_instructions,
                    },
                },
            ]
                    ...
                    if isinstance(e, ExceptionGroup):
                        ...
                        if e.exceptions:
                            ...
                            actual_error = ValueError(
                                "Empty ExceptionGroup caught"
                            )  # Fallback

                    last_error = actual_error  # Store the original or extracted error
                    log(
                        "info",
                        f"DEBUG: 在重试循环中捕获异常: type={type(e)}, 可能被包装. 使用: type={type(actual_error)}, value={repr(actual_error)}",
                    )  # 更新调试日志
                    log(
                        "info",
                        f"尝试 {i + 1} ('{attempt['name']}') 失败: {actual_error}",
                    )  # 记录实际错误
                    if i < len(attempts) - 1:
            ...
            try:
                result = await make_gemini_call(
                    current_model_name, current_prompt_func, current_config
                )
                return result
            except Exception as e:
                # 处理非自动模型可能出现的错误
...
def health_check(api_key: str = Depends(get_api_key)):
    ...
    return {
        ...
        "credentials": {
            ...
            "files": [
                os.path.basename(f) for f in credential_manager.credentials_files
            ],
